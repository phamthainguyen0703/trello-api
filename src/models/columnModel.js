...
// cập nhật push cardId vào cuối mảng CardOrderIds(search doc mongoDB)
...
const update = async (columnId, updatedColum) => {
  try {
    // lọc field không cho phép update
    Object.keys(updatedColum).forEach((fieldName) => {
      if (INVALID_UPDATE_FIELDS.includes(fieldName)) {
        delete updatedColum(fieldName);
      }
    });

    if (updatedColum.cardOrderIds) {
      updatedColum.cardOrderIds = updatedColum.cardOrderIds.map(
